...
ELEMENTS = ["猫", "狼", "蛇", "马", "棺材", "酒", "老人", "刀", "死亡", "兔子", "鸡", "牙齿",
            ...
            # 比值档阈探针元素（r9）
            "乌檀", "白檀", "青檀", "乌锭", "白锭", "青砖", "白砖", "青锭",
            # 强档样本量/反例阈值探针元素（I-1）
            "石砧", "木砧", "灰砧", "银锭", "锡锭"]
CONTENT_CAP = 3000
# 比值档阈探针用的判词词表（吉/凶各取前 N 条拼成不同句；都是窄词表术语，
# 因此句子必然被判为单向吉/凶 —— 这正是探针需要的行为）
JI_WORDS = ["得财", "主吉", "大吉", "有财", "富贵", "吉利", "喜事", "顺遂"]
# ⚠️ 凶向词必须**命中窄词表**（XIONG_STRONG）才会被判为凶 —— 第一版用了
# 「恐有官非 / 家宅不宁 / 财物有失」等 5 个不命中窄表的措辞，探针读数整体错位
# （乌檀 5:1 变 5:0 → 大吉）。措辞还要**避开已有模板**（否则会顶坏族旋钮探针）。
XIONG_WORDS = ["恐有血光", "逢疾病", "主丧事", "遇官司", "主离别", "多有损失",
               "犯小人", "遭灾祸"]           # 夹具内正文截断上限（控制入库体积；确定性）
...
CRAFTED = [
    ...
    *[{"title": f"梦见{e}的预兆{i}", "content": f"梦见{e}，是不祥之兆。",
    ...
    {"title": "梦见锡锭", "content": "梦见锡锭，得财。梦见锡锭，主吉。梦见锡锭，大吉。"
     ...
     "source": "fixture_crafted"},
    # ⑪ 比值档阈四个旋钮（r9）—— 每个都有**语料级两侧探针**：
    #   · 0.8 ：「乌檀」5:1=0.833 → 吉多于凶（上侧）；「白檀」7:2=0.778 → 吉（下侧）
    #   · 0.62：「青檀」4:2=0.667 → 吉（能测「提高阈值」；**下侧无探针** ——
    #           r<0.62 的强档样本必然 |差|<2 且 tot<10，被 C3 拦成中性，结构上不可得）
    #   · 0.42：「乌锭」5:7=0.4167 → 凶多于吉（下侧）；「白锭」6:8=0.4286 → 吉多于凶（上侧）
    #   · 0.25：「青锭」2:7=0.222 → 凶（下侧，比 1:4=0.2 更贴阈值）；
    #           「白砖」2:5=0.286 → 凶多于吉（上侧）
    *[{"title": f"梦见{el}", "content": "".join(
        f"梦见{el}，{w}。" for w in (JI_WORDS[:nj] + XIONG_WORDS[:nx])),
       "source": "fixture_crafted"}
      for el, nj, nx in (("乌檀", 5, 1), ("白檀", 7, 2), ("青檀", 4, 2),
                         ("乌锭", 5, 7), ("白锭", 6, 8), ("青砖", 1, 4),
                         ("白砖", 2, 5), ("青锭", 2, 7))],
]
...
def self_check(entries: list, expected: dict) -> None:
    ...
    # ⑧b 展示层（r9 裁决④）：公式句**不当释义依据**；只有无替代时才退化并标档次
    fk = set(expected["formula_keys"])
    for e, d in expected["elements"].items():
        if d["gloss_raw"]:
            assert not any(B.sentence_key(s) in fk for s in (d["gloss_raw"],)), \
                f"⑧b {e} 的释义依据仍是公式句：{d['gloss_raw'][:40]}"
    # 夹具里 `刀光` 的池**只有**一条公式句 → 必须走退化 + 标 `_formula_only`
    assert expected["elements"]["刀光"]["gloss_evidence"].endswith("_formula_only"), \
        ("⑧b 退化档次未标注：", expected["elements"]["刀光"]["gloss_evidence"])

    # ⑦ 比值档阈四个旋钮（r9）：两侧探针的档位必须落在预期一侧
    for e, want in (("乌檀", "吉多于凶"), ("白檀", "吉"), ("青檀", "吉"),
                    ("青砖", "凶"), ("白砖", "凶多于吉"), ("青锭", "凶"),
                    ("青砖", "凶"), ("白砖", "凶多于吉")):
        assert el[e]["luck"] == want, f"⑦ 比值探针 {e} 档位异常：{el[e]['luck']} ≠ {want}"
