...
# `_AGE_CLAIM_RES[2]` 右侧同位置已含「时」）为对齐事实源。
# k35-fix（复审 Important-1）：初版把裸字符「时」直接并入排除面 → 放宽面超出
# 时点语境（`虚岁33岁时运不济/时，我事业起飞/时已经结婚/今年我虚岁33岁时运不济`
# 等「当前年龄帧 + 时」句被放行）。现改为**显式时点语境白名单**：`岁时` 之后
# 允许句读与主语/已然副词，但必须紧跟换运**时点动**词（`_K35_TIME_POINT_VERBS`）；
# `时运不济`「时来运转」等非时点语境重新拦回。语义边界：放行的只是「N 岁时开始
# 换入第 M 步大运」这一时点限定，N 仍被前缀型/完成体/裸岁句读型三族覆盖。
_K35_TIME_POINT_VERBS = "进入|步入|踏入|转入|换入|换运|走|开始|交运|起运"
_AGE_CLAIM_RES = (
    re.compile(r"(?:今年|现在|如今|目前|本人|我已经|我今年|命主|用户|你今年|你现在)"
               r"\s*(?:周岁|虚岁)?\s*(\d{1,2})\s*岁"),
    re.compile(r"(?<![\d岁到从走换进交止起至后])(?:周岁|虚岁)\s*(\d{1,2})\s*岁"
               r"(?![\d到至起走换进交止后～~\-–—－]"
               r"|时(?=[，,、]?\s*(?:你|我|他|她|命主|本人|用户)?\s*"
               r"(?:已|已经|将|将要|正|就|便|会|即将|刚)?\s*"
               r"(?:" + _K35_TIME_POINT_VERBS + r")))"),
    ...
    re.compile(r"(?<![\d岁至→>到从走换止起进交\-–—～~])(\d{1,2})\s*岁"
               r"(?=[，。！？；、,.!?\s]|$)"),
    # k38（T102 断言实现缺口）：单位与数字之间夹副词型当前年龄声明——
    # 「今年是2026年，你虚岁已经28岁了」（产品实际答对，旧 6 条 pattern 全部要求
    # 「虚岁/周岁」与数字相邻 → require_mention 判「未提及 27/28 岁」）。
    # 副词**必须**出现（不可写成可选）：一旦可选，本型退化为无护栏的裸
    # 「虚岁N岁」，把大运段的时间状语变体（「虚岁33岁时换入乙丑」——尾部护栏
    # 只排除段连接词/区间符，不排除「岁时」）误判成当前年龄声明，污染
    # T101/T103 同族契约（k15/k35 已用前后双向排除固化该防线）。
    # k38-M5（审查实测口径修正）：本注释原举的大运段端点形态（「虚岁23岁到
    # 32岁」「从虚岁33岁起」）在**现有前后护栏**下就已不命中（左/右分别有
    # 岁·到·从 排除），真正需要副词必填挡住的是上面的「岁时」变体——
    # 忠实于实测，勿再据旧注释推断护栏边界。
    # 左右护栏与 pattern2 同口径（左：段连接/换运动词；右：段连接/区间符）。
    re.compile(r"(?<![\d岁到从走换进交止起至后])(?:周岁|虚岁)\s*"
