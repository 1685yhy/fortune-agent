...
1. **⚠️ 我发现的语义张力，已按"不丢数据 + 不制造死数据"处置，但请你确认**：
   **③ 删掉了语料唯一的 LLM 消费者**（那条准确率用例），于是 ② 内联的 54 条**在 LLM 侧没有消费者了**。
   两条裁决单独看都成立，合起来会产生"内联了一堆没人用的数据"。
   我的处置：把它保留为**固定、分层、可复现的种子语料**，并由 §5 的锁在**常规门禁**里
   钉住形状与抽样（不是死数据 —— 它有断言在管；也不是活能力 —— 已明写「该能力随模块一起移除」）。
   **若你认为该语料无保留价值**，请指示，我可整段删除（那样 ② 的"内联"也随之作废，只剩 ③ 的删除效果）。
