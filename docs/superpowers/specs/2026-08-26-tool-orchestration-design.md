...
分三批独立上线，每批独立验证、零回归护栏：

- **批次 0（地基，先于一切）**：测试基线建立 + 现象清单校准——盘点全量测试、查明 22 个历史失败原因并修复/归类，固化为"基线复跑命令 + 基线结果文件"作为后续每批改动的对照尺子；同时收集 PM 真实不满意的对话例子逐条复现归类，校准批次 1 内容
...
**为什么**：现状 test_bot.py 基线是"22 failed / 43 passed"——22 个历史失败未查明原因，改完无法区分"新引入问题"与"本就损坏"。PM 要求先建立可靠测试基线再动工。

### 0.1 测试基线盘点（调查代理已启动，产出 `.superpowers/sdd/test-baseline-audit.md`）

1. 全量跑 `tests/`，记录每文件 通过/失败/错误/跳过
2. 22 个失败逐个归类：
   - A 环境依赖缺失（faiss 未装/模型文件/网络）→ 记录环境要求，不修代码
   - B 测试与实现不一致（旧断言未随近期 F1/F2 改动同步）→ **修复测试**（实现为准）
   - C 测试自身缺陷（mock 不完整/fixture 错误/flaky）→ **修复测试**
   - D 真实 bug（被测代码确实错）→ 报告 PM，单独排期修，不混入基线
   - E 需外部资源（真库/真 API）→ 标记为"需环境"测试，基线命令跳过或注明
3. 修复 B/C 类（派 implementer，逐条验证），产出**干净基线**：失败集收敛到 {A/E 类 + 明确标注的预期失败}，其余全绿

### 0.2 基线固化

- 新增 `tests/run_baseline.sh`（或文档化命令）：一条命令复跑基线 + 输出结果文件 `.superpowers/sdd/baseline-YYYYMMDD.md`
- 基线结果文件含：命令、环境要求（Python 路径/依赖）、全量数字、失败清单（仅 A/E/预期）
- **后续每批改动的验收标准 = 基线对照**：通过集不减少、失败集逐名不新增（除新批次的预期变化外）

### 0.3 现象清单校准（PM 提供 5~10 个真实不满意的对话例子）

- 逐条在生产/测试环境复现，归类根因在"意图理解 / 信息累积 / 工具选择 / 返回格式"哪一层
- 输出 `.superpowers/sdd/phenomenon-list-YYYYMMDD.md`：每个例子的 原文 → 现行为 → 复现步骤 → 根因层 → 对应批次 1 哪项改动
- 若发现例子根因不在批次 1 范围（如纯意图理解问题），在批次 1 计划中补充对应任务——**方案按真实痛点校准，不按外部做法照搬**
...
- 批次 0 先行：基线未固化不得开批次 1；基线对照不合格的改动不上线

## 验收标准

- 批次 0：全量测试盘点完成；B/C 类失败修复后基线干净（失败集仅剩 A/E/预期且逐名有据）；基线命令+基线文件固化；现象清单 5~10 条逐条复现归类，批次 1 计划按校准后内容编写
- 批次 1：选工具零"文本标签残留"（tool_calls.py:18-23 的老修复点可移除兜底验证）；参数非法不执行；超时/失败有兜底文案；意图与工具说明书同源；基线对照无回归
- 批次 2：普通排盘端到端 ≤15s（现 20~40s）；快通道场景 ≤2s；单请求 LLM 调用 ≤4 次；通用问答语义缓存命中返回 <1s；基线对照无回归

## 待拍板/观察项（不影响开工）

- PM 提供现象清单 5~10 个真实不满意的对话例子（批次 0 前置，可后补但批次 1 计划要等它）
