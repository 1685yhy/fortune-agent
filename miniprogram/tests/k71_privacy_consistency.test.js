  ...
  .concat([
    ...
    { re: /(注销|销号)[^。]{0,24}不可恢复|不可恢复[^。]{0,16}(注销|销号)/,
      why: '与上一条同源：注销存在"人工从备份尝试找回"这一路径，'
        ...
        + '应写"无法自助恢复"' },
    /* ── k84 新增（必修4）：**删除语境**里的"不可恢复"绝对句 ──────────────────
       上面那条锚在「注销|销号」上 ⇒ 不带注销词的"删除后不可恢复"整类逃逸。
       k84 实测的 5 个行位点（改前全绿 = 全部逃逸）：
         pages/history/history.wxml:140,143   （旧记录整批清除 / 本机+服务端一起删）
         pages/dreams/dreams.wxml:113         （解梦手记单条删除）
         pages/persons/persons.js:356         （档案删除 wx.showModal content）
         pages/chat/chat.js:1287              （对话单条删除 wx.showModal content）
       本条的**语境限定**（删除/清空/清除/注销/销号/移除 就近出现）让它无需任何
       文件级豁免就能精确抓住这一类，同时不误伤 utils/api.js 里 k47-D 重登链路的
       技术状态名（见 why ③ 的边界登记）。 */
    { re: /(删除|清空|清除|注销|销号|移除)[^。；\n]{0,20}不可恢复|不可恢复[^。；\n]{0,20}(删除|清空|清除|注销|销号|移除)/,
      why: '删除语境里的绝对句"不可恢复"同为**过度断言**（本仓统一口径 = "无法自助恢复"，'
        + 'k75 主线落点：pages/settings/settings.wxml:206/215/249、pages/privacy/privacy.wxml:12、'
        + 'privacy.md:102/114）：① 代码反证 —— 每日备份保留最近 14 份'
        + '（scripts/backup_db.py:DEFAULT_KEEP = 14），删除当刻的记录仍可能落在最近一份备份里，'
        + '"绝对不可恢复"不是事实；② 单条记录删除**没有自助恢复入口**，故"无法自助恢复"是准确'
        + '且不缩水的说法；注意单条删除**没有保留期**，不许把注销专属的"保留期内可申请人工从'
        + '备份尝试找回"长句套过来（那是新的过度承诺）；'
        + '③ 边界（如实登记，k84 按词法级实测校正过条数）：miniprogram/utils/api.js 的'
        + '"不可恢复"实为 **8 处 / 8 行**（333/342/354/372/396/416/436/448；去注释后只剩 2 处'
        + '字面量，即 396/416）—— 全是 k47-D 重登链路的技术状态名"不可恢复错误/不可恢复短路"，'
        + '不是数据删除断言。本条按**删除语境**限定 ⇒ 对它们**一处都不命中**（实测：全文与'
        + '字面量单元命中数均为 0），故**不需要**给 api.js 开文件级豁免（文件级豁免 = 给整个'
        + '文件开口子，本仓明确不鼓励；守卫只扫 JS 字符串字面量、跳过注释，边界更窄）' },
    { re: /期满彻底删除|期满后彻底删除/,
