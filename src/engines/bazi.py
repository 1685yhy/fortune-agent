...
CITY_LONGLAT = {
    ...
    # 吉林
    "长春": (125.35, 43.88), "吉林市": (126.55, 43.84), "榆树": (126.53, 44.83),
    # 黑龙江
...
class BaziEngine:
    ...
    def _city_longitude(city) -> Optional[float]:
        ...
        1. 原样精确命中（CITY_LONGLAT 键，现有行为）；
        2. 去「市」「省」尾缀后命中（现有行为）；
        3. 剥行政区划前缀（XX省/XX自治区/XX特别行政区；直辖市『北京市』
           类由尾缀规则 2 覆盖）后，剩余部分再走 1+2；
        4. 最长后缀兜底：输入以表中某键结尾（键或以键+'市'结尾）→ 取最长的
           匹配键（防 '吉林市' 与 '长春' 类混淆）；键长相等 → 取输入串中
           位置靠后者（城市名一般在省名之后）；
        ...
        # 层 4：最长后缀兜底（键或以键+'市'结尾 → 取最长键；等长取靠后者）
        _tail = []
    ...
    def calculate(self, year: int, month: int, day: int,
        ...
        - daylight_saving=False 默认关=现行为零变化；开启时若出生时刻（北京时间，
          ...
          还原真实时间，再走真太阳时修正（solar_time 开时）与排盘
          （问真客户端夏令时开关口径）。
