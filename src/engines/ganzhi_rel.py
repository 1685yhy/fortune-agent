...
- 争合：两干争合一干（被合者非日主，如 两庚争合乙）
- 妒合：两干争合日主（被合者为日主，问真口径简化：多干合一日干）

说明：盖头/截脚为单柱柱内性质（a、b 各自判定），争合/妒合需四柱（pillars）参与；
BaziResult.rel_with 为"点大运/流年看干支关系"的入口（大运/流年 vs 原局各柱）。
"""
...
class RelationItem:
    ...
    type:    关系类型（伏吟/反吟/盖头/截脚/争合/妒合/天克地冲/天克/地冲）
...
def _he_contend(ag: str, bg: str, pillars: Optional[List[str]]) -> List[RelationItem]:
    """争合/妒合：多干争合一干（a/b 天干与四柱天干并集，两干合一干）。

    被合干为日主（pillars[2][0]）→ 妒合（问真口径简化：多干合一日干）；
    被合干非日主 → 争合。
    """
    if not pillars or len(pillars) < 4:
        return []
    day_gan = pillars[2][0]
    stems = [ag, bg] + [p[0] for p in pillars]
    out = []
    for target in stems:
        partners = [s for s in stems if s != target and HE.get(s) == target]
        if len(partners) >= 2:
            desc = "、".join(partners[:3]) + "两干争合" + target
            if target == day_gan:
...
def analyze_relations(a: str, b: str, pillars: Optional[List[str]] = None) -> List[RelationItem]:
    ...
    :param b: 干支二，如 "己未"
    :param pillars: 四柱列表（争合/妒合判定需要；缺省则跳过争合/妒合）
    :return: 关系列表，type ∈ 伏吟/反吟/天克地冲/天克/地冲/盖头/截脚/争合/妒合
    """
    ...
    # 争合/妒合：多干合一干（日主被争合 → 妒合）
    out.extend(_he_contend(ag, bg, pillars))
