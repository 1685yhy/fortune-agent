...
def normalize_hour(hour: Optional[int], minute: Optional[int] = None,
                   clock_signal: bool = False) -> int:
    """时辰归一化：0-11 视为时辰序号（小程序 picker 的取值）映射为时钟小时；
    12-23 视为时钟小时原样返回；缺省取 12（午时）。

    clock_signal / minute（k19 分钟精度）：调用方声明该 hour 为**钟表时钟
    小时**（表单选了精确钟表时间）时——0-23 全部原样直通引擎做真太阳时
    校准，不再把 0-11 当时辰序号（10:55 场景：hour=10 + minute=55 必须
    直通；hour=10 + minute=0 + birthClock 声明的 10 点整同理）。判定：
    显式 clock_signal=True 或 minute>0。旧调用方不带两者 → 行为零变化。
    """
    if hour is None:
        return 12
    try:
