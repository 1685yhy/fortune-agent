...
## ⚠️ 本锁保护的是「**存档完整性**」，不是「活资产」

被锁的 54 条语料是**存档**：它为一个**从未接入产品**的能力（情绪/人设判定）而准备，
**唯一消费者已随被测模块（`src/engines/mood_detector.py`，k62 `2a67833` 拆除）
一起删除 —— 现在没有任何代码在用它**。所以本文件的断言**不是**在保护某个在用的
能力，而是在保护这份存档**没有被悄悄改写、也没有被偷偷接回已删模块**
（接回去 = 合并/收集即 ERROR，且会让读者误以为该能力还在）。

⇒ 读这些断言时不要得出"还有个能力在用它"的结论。
**移除触发条件**：若该能力在后续规划中不再重建，则本存档与其锁一并移除
（即连同 `tests/test_k61_e2e_smoke.py` 里的 `LABELED_TEST_CASES` 一起删）。
"""
...
def test_corpus_is_defined_in_place_not_imported():
    ...
    ⚠️ 保护的是**存档完整性**，不是活资产（该能力的消费者已随模块删除，
    见模块 docstring）—— 别把它读成"还有个能力在用它"。
    """
