...
def test_r13_engine_wushi_noon_hour_pillar_anchor():
    """校准锚点：2019-03-15 午时 北京 → 时柱甲午（R1-3：午时序号 6 → 时钟小时
    12 取中点，修正后 ≈11:35 仍午时——默认开口径下锚点稳定命中；修复前 11 点
    映射 → 修正落回巳时 → 癸巳时，与问真/权威排盘不一致）。"""
    eng = BaziEngine()
    noon = eng.calculate(2019, 3, 15, 12, 0, "北京", "男")
    assert noon.bazi[3] == "甲午", noon.bazi
    # R2-4（真太阳时默认开=产品口径）：11:00 默认即修正（10:35 → 巳时癸巳——
    # R1-3 期锁定的错位窗口，边界时刻修正固有属性，用户可关开关对比）；
    # 显式关（solar_time=False）= 北京时间直排 → 午时甲午（问真默认态口径）
    assert eng.calculate(2019, 3, 15, 11, 0, "北京", "男").bazi[3] == "癸巳"
    off11 = eng.calculate(2019, 3, 15, 11, 0, "北京", "男", solar_time=False)
    assert off11.bazi[3] == "甲午"  # 用户关闭后=北京时间直排（问真默认态）
