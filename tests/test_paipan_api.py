...
注意：R2-1（问真口径）真太阳时默认关——传「北京」直接按北京时间排盘，
起运分解与 city="" 完全一致（修正开口径的分钟级差异已不再是产品默认）。
"""
import os
import sys
...
def test_paipan_full_chart_anchors():
    ...
    # ── L1：起运 / 交运 / 司令 ──
    # R2-1（问真口径默认关）：午时序号 6 → 时钟小时 12 → 12:25 北京时间直接
    # 排盘 → 起运 2年4月27天0时（修正开口径为 2年4月26天1时——R1-3 校准值，
    # 默认关实测 2026-09-02；时柱壬午锚点双口径一致）。
    assert body["qiyun_desc"] == "出生后2年4月27天0时起运"
