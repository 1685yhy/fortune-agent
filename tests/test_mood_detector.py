...
class TestRealAPI:
    ...
    #
    # ⚠️ 另外一条**定级事实（k61 r5 更正，控制方/用户指正）**：本模块
    # （`src/engines/mood_detector.py`）在 **`src/` 下被引用 0 次** —— 只有测试引用它，
    # **没有生产调用点**。所以这里的任何读数（含上面的 0.667 / 0.556）
    # **只是该模块自身的测试读数，度量不到任何产品行为**，
    # **不得**被引用为"产品准确率"或"用户体验下降"的证据。
    # 核实方法见报告《本模块的调用面核实》一节。
